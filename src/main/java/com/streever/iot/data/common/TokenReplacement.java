...
package com.streever.iot.data.common;
...
public class TokenReplacement {
    ...
    public Map<String, Object> getDefaultTokens(List<String> text) {
        Map<String, Object> defaultTokens = new HashMap<String, Object>();
        ListIterator<String> listIterator = text.listIterator();
        while (listIterator.hasNext()) {
            String line = listIterator.next();
//        for (String line: listIterator.text) {
            Matcher matcher = pattern.matcher(line);
            String newLine = line;//new StringBuilder();
            while (matcher.find()) {
                String token = matcher.group(1);
                String[] tParts = token.split(":");
                if (tParts.length < 2) {
                    defaultTokens.put(tParts[0], null);
                } else if (tParts.length == 2) {
                    defaultTokens.put(tParts[0], tParts[1]);
                    newLine = line.substring(0, matcher.start()) + "${" + tParts[0] + "}" + line.substring(matcher.end(), line.length());
                    listIterator.set(newLine);
                } else {
                    defaultTokens.put(token, null);
                }
            }
        }
